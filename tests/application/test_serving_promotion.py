...
from application.test_fedper_bundle_runtime import build_test_bundle
...
class ServingPromotionTests(unittest.TestCase):
    def test_promotion_is_derived_immutable_and_strictly_loadable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            research, _ = build_test_bundle(root)
            ...
            report = {
                ...
                "selected": {
                    ...
                    "graph_protocol": (
                        "rolling-window-v1:duration=15s:max-flows=100:stride=1:"
                        "lateness=1s"
                    ),
