...
function setControlPane(mode) {
  const traffic = mode === "traffic";
  $("replay-tab").classList.toggle("active", !traffic);
  $("traffic-tab").classList.toggle("active", traffic);
  $("replay-tab").setAttribute("aria-selected", String(!traffic));
  $("traffic-tab").setAttribute("aria-selected", String(traffic));
  $("replay-pane").hidden = traffic;
  $("traffic-pane").hidden = !traffic;
  $("replay-pane").classList.toggle("active", !traffic);
  $("traffic-pane").classList.toggle("active", traffic);
  $("control-mode-badge").textContent = traffic ? "LIVE TRAFFIC" : "VALIDATION";
  $("control-mode-badge").classList.toggle("traffic", traffic);
  if (traffic && state.trafficCatalog == null) loadTrafficProfiles();
}

function trafficStatusLabel(value) {
  return ({
    "waiting-for-release": "Chờ pipeline",
    running: "Đang phát",
    completed: "Hoàn tất",
    cancelled: "Đã dừng",
    failed: "Thất bại"
  })[value] || value || "Chưa chạy";
}

function scientificStatusLabel(value) {
  return ({
    candidate: "Candidate",
    "control-not-class-equivalent": "Benign control",
    "blocked-target-not-ready": "Khóa · chưa có target",
    "unsupported-dataset-artifact": "Khóa · không tái tạo được"
  })[value] || value;
}

function trafficRunActive() {
  return ["waiting-for-release", "running"].includes(state.trafficRun?.status);
}

function updateTrafficActions() {
  const profile = state.selectedTrafficProfile;
  $("start-traffic-button").disabled = state.trafficBusy || trafficRunActive() || !profile?.execution_enabled;
  $("stop-traffic-button").disabled = state.trafficBusy || !trafficRunActive();
...
function selectTrafficProfile(profile) {
  state.selectedTrafficProfile = profile;
  document.querySelectorAll(".traffic-profile-button").forEach((button) => {
    button.classList.toggle("active", button.dataset.id === profile.id);
  });
  const detail = $("traffic-detail");
  detail.className = "traffic-detail";
  detail.replaceChildren();
  const heading = document.createElement("div");
  heading.className = "traffic-detail-heading";
  const name = document.createElement("strong");
  name.textContent = profile.reference_class;
  const status = document.createElement("span");
  status.className = `profile-status ${profile.execution_enabled ? "enabled" : "blocked"}`;
  status.textContent = scientificStatusLabel(profile.scientific_status);
  heading.append(name, status);
  const meta = document.createElement("div");
  meta.className = "traffic-meta";
  const observables = profile.expected_observables || {};
  [
    ["Cơ chế", profile.mechanism],
    ["Target group", profile.target_group],
    ["Cổng đích", profile.destination_port],
    ["Event / khoảng cách", `${profile.events} / ${(profile.interval_ms / 1000).toFixed(1)}s`],
    ["Protocol", (observables.protocols || []).join(", ") || "—"],
    ["Service / state", `${(observables.services || []).join(", ") || "unknown"} / ${(observables.connection_states || []).join(", ") || "—"}`]
  ].forEach(([label, value]) => {
...
function renderTrafficCatalog(catalog) {
  state.trafficCatalog = catalog;
  const selector = $("traffic-profiles");
  selector.replaceChildren();
  catalog.profiles.forEach((profile) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = `traffic-profile-button${profile.execution_enabled ? "" : " blocked"}`;
    button.dataset.id = profile.id;
    button.dataset.enabled = String(profile.execution_enabled);
    button.setAttribute("aria-disabled", String(!profile.execution_enabled));
    const label = document.createElement("strong");
    label.textContent = profile.reference_class;
    const caption = document.createElement("small");
    caption.textContent = profile.execution_enabled ? `${profile.events} events · port ${profile.destination_port}` : "Không khả dụng";
    button.append(label, caption);
    button.addEventListener("click", () => selectTrafficProfile(profile));
    selector.append(button);
  });
  const initial = catalog.profiles.find((profile) => profile.execution_enabled) || catalog.profiles[0];
  if (initial) selectTrafficProfile(initial);
}

function renderTrafficRun(record) {
  state.trafficRun = record;
  const panel = $("traffic-run-status");
  if (!record) {
    panel.className = "traffic-run-status empty-state";
    panel.textContent = "Chưa có traffic run.";
    updateTrafficActions();
    return;
  }
  panel.className = `traffic-run-status ${record.status || "unknown"}`;
  panel.replaceChildren();
  const title = document.createElement("div");
  title.className = "traffic-run-heading";
  const name = document.createElement("strong");
  name.textContent = record.profile_id;
  const status = document.createElement("span");
  status.textContent = trafficStatusLabel(record.status);
  title.append(name, status);
  const counters = document.createElement("div");
  counters.className = "traffic-run-counters";
  const collector = record.pipeline?.collector || {};
  [
    ["Đã phát", `${record.succeeded || 0}/${record.attempted || 0}`],
    ["Collector", `${collector.accepted || 0}/${collector.received || 0}`],
    ["Predicted", collector.predicted || 0],
    ["Drop / lỗi", `${collector.late_dropped || 0}/${(collector.inference_failures || 0) + (collector.alert_sink_failures || 0)}`]
  ].forEach(([label, value]) => {
    const cell = document.createElement("span");
    cell.textContent = label;
    const content = document.createElement("b");
  ...
  panel.append(title, counters);
  updateTrafficActions();
}

async function loadTrafficProfiles() {
  $("traffic-agent-dot").className = "status-dot pending";
  $("traffic-agent-label").textContent = "Đang kết nối agent";
  try {
    const catalog = await json("/api/traffic-profiles");
    renderTrafficCatalog(catalog);
    $("traffic-agent-dot").className = "status-dot ready";
    $("traffic-agent-label").textContent = "Traffic agent ready";
    await pollTrafficRun();
  } catch (error) {
    $("traffic-agent-dot").className = "status-dot error";
    $("traffic-agent-label").textContent = `Agent offline · ${error.message}`;
    state.trafficCatalog = null;
    updateTrafficActions();
  }
}

async function startTrafficRun() {
  const profile = state.selectedTrafficProfile;
  if (!profile?.execution_enabled || trafficRunActive()) return;
  ...
  try {
    ...
    $("traffic-agent-dot").className = "status-dot error";
    $("traffic-agent-label").textContent = error.message;
  } finally {
...
async function stopTrafficRun() {
  if (!trafficRunActive()) return;
  state.trafficBusy = true;
  updateTrafficActions();
  try {
    const body = await json("/api/traffic-runs/current", {method: "DELETE"});
    renderTrafficRun(body.run || null);
    $("traffic-agent-label").textContent = "Traffic đã dừng";
  } catch (error) {
    $("traffic-agent-dot").className = "status-dot error";
    $("traffic-agent-label").textContent = error.message;
  } finally {
...
async function pollTrafficRun() {
  if (state.trafficCatalog == null || state.trafficBusy) return;
  try {
    const body = await json("/api/traffic-runs/current");
    renderTrafficRun(body.run || null);
    $("traffic-agent-dot").className = "status-dot ready";
    $("traffic-agent-label").textContent = trafficRunActive() ? "Traffic agent đang phát" : "Traffic agent ready";
  } catch (error) {
    $("traffic-agent-dot").className = "status-dot error";
    $("traffic-agent-label").textContent = `Agent offline · ${error.message}`;
  }
...
async function boot() {
  ...
  $("replay-tab").addEventListener("click", () => setControlPane("replay"));
  $("traffic-tab").addEventListener("click", () => setControlPane("traffic"));
  $("run-replay-button").addEventListener("click", runScientificReplay);
  $("traffic-refresh-button").addEventListener("click", loadTrafficProfiles);
  $("start-traffic-button").addEventListener("click", startTrafficRun);
  $("stop-traffic-button").addEventListener("click", stopTrafficRun);
  $("clear-events").addEventListener("click", clearMonitor);
  setInterval(pollMonitor, 1000);
  setInterval(pollTrafficRun, 1000);
